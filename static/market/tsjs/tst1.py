...
url = "https://query1.finance.yahoo.com/v7/finance/download/MSFT?period1=" + str(start) + "&period2=" + str(end) + "&interval=1d&events=history&includeAdjustedClose=true"
print (url)
r = urllib2.urlopen(url).read()
file = BytesIO(r)
df2 = pd.read_csv(file,index_col='Date',parse_dates=True)

ax1 = df2['Adj Close'].plot(color='blue', grid=True, label='')
ax2 = df1['Adj Close'].plot(color='red', grid=True, label='',secondary_y=True)
